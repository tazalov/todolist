...
describe('fetchTasksByTodolistId thunk', () => {
  it('set of actions for a successful request is correct', async () => {
    ...
    } as unknown as AxiosResponse<ItemsResponseT<TaskT>>

    tasksAPIMock.getTasks.mockReturnValue(Promise.resolve(result))
  ...
  it('set of actions for a bad request is correct', async () => {
    ...
    tasksAPIMock.getTasks.mockReturnValue(Promise.reject(new Error('some error')))
